...
const chain=new KorekChain(),jobs=new Map(),sync={mode:syncMode,state:"Idle",peers:0,note:"Standalone prototype: P2P chain synchronization is not implemented"};
...
const nodeStatus=()=>({...chain.status(),version:NODE_VERSION,chain:chainName,name:nodeName,validator,nodeIdentity,minerProtocol:MINER_PROTOCOL,minerListenPort:minerPort,maxBlocksPerRequest:maxBlocks,sync,mining:{enabled:miningEnabled,rewardsAddress}});
...
const apiServer=createServer(async(req,res)=>{try{const url=new URL(req.url,`http://${req.headers.host}`);
 ...
 if(req.method==="POST"&&url.pathname==="/api/transactions"){const tx=chain.addTransaction(await body(req));chain.sealPending();return json(res,201,chain.transaction(tx.id),true)}
 if(req.method==="POST"&&url.pathname==="/api/faucet"){const input=await body(req);return json(res,201,chain.claimFaucet(input.address),true)}
 ...
 if(req.method==="POST"&&url.pathname==="/api/mine")return json(res,201,mineInput(await body(req)),true);
...
apiServer.listen(NETWORK.apiPort,()=>{console.log(`KOREK node ${NODE_VERSION} '${nodeName}'`);console.log(`Chain: ${chainName} · API/explorer: http://localhost:${NETWORK.apiPort}`);console.log(`Sync: ${sync.state} · peers: ${sync.peers} (${sync.note})`);if(nodeIdentity)console.log(`Node identity: ${nodeIdentity}`);if(rewardsAddress)console.log(`Configured rewards: ${rewardsAddress}`)});
minerServer.listen(minerPort,()=>console.log(`Miner protocol ${MINER_PROTOCOL} listening on http://localhost:${minerPort}`));
if(miningEnabled)setInterval(()=>{if(mining)return;mining=true;try{const block=mineInput({rewardsInnerHash});console.log(`Mined block #${block.height} · ${block.reward} atomic KRK · ${block.hash}`)}catch(error){console.error(`Mining error: ${error.message}`)}finally{mining=false}},NETWORK.rewardBlockTimeMs);
