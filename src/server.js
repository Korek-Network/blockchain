...
const nodeSnapshot=()=>({bundleVersion:2,networkId:NETWORK.networkId,chain:chain.snapshot(),compute:computeMarket?.snapshot()||preservedCompute,validators:validatorRegistry?.snapshot()||preservedValidators});
...
const cleanupWork=(now=Date.now())=>{for(const[id,item]of workTemplates)if(item.expiresAt<now)workTemplates.delete(id);for(const[id,expires]of usedTemplates)if(expires<now)usedTemplates.delete(id);for(const[id,seen]of workRate)if(now-seen>60_000)workRate.delete(id)};
...
const apiServer=createServer(async(req,res)=>{try{const url=new URL(req.url,`http://${req.headers.host}`);
 ...
 const computeRoute=url.pathname.match(/^\/api\/compute\/jobs\/([0-9a-f]{64})(?:\/(claim|result|vote|cancel|expire))?$/i);
 if(computeRoute){requireCompute();const[,jobId,action]=computeRoute;if(req.method==="GET"&&!action)return json(res,200,computeMarket.job(jobId),true);if(req.method==="POST"&&action){const input=await body(req);const job=await persistCompute(()=>action==="claim"?computeMarket.claimJob(jobId,input):action==="result"?computeMarket.submitResult(jobId,input):action==="vote"?computeMarket.vote(jobId,input):action==="cancel"?computeMarket.cancelJob(jobId,input):computeMarket.expireJob(jobId));return json(res,200,job,true)}}
