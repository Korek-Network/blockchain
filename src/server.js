...
const computeEnabled=process.env.KOREK_COMPUTE_MVP==="1";if(computeEnabled&&p2pRequested)throw new Error("KOREK_COMPUTE_MVP is single-node only until compute transactions have full P2P consensus validation");
...
let rewardsAddress=null,nodeKey=null;if(rewardsInnerHash)rewardsAddress=wormholeAddressFromInnerHash(rewardsInnerHash);if(miningEnabled&&!rewardsAddress)throw new Error("Built-in mining requires --rewards-inner-hash <64 hex characters>");
...
const savedBundle=[1,2,3].includes(savedState?.bundleVersion)&&savedState.networkId===NETWORK.networkId?savedState:null;
let chain=savedBundle?KorekChain.fromSnapshot(savedBundle.chain):savedState?KorekChain.fromSnapshot(savedState):new KorekChain();
...
const computeMarket=computeEnabled?new ComputeMarket(chain,validatorRegistry,preservedCompute):null;
const nodeSnapshot=()=>({bundleVersion:3,networkId:NETWORK.networkId,chain:chain.snapshot(),compute:computeMarket?.snapshot()||preservedCompute,validators:validatorRegistry?.snapshot()||preservedValidators});
const saveNodeState=()=>stateStore.save((computeEnabled||preservedCompute||preservedValidators)?nodeSnapshot():chain.snapshot());
const workTemplates=new Map(),usedTemplates=new Map(),requestNonces=new Map(),workRate=new Map(),workStats={templatesIssued:0,accepted:0,rejected:0,duplicates:0,stale:0};
...
const apiServer=createServer(async(req,res)=>{try{const url=new URL(req.url,`http://${req.headers.host}`);
 ...
 const validatorRoute=url.pathname.match(/^\/api\/compute\/validators\/(krk1[0-9a-f]{40})(?:\/(exit|withdraw))?$/i);
 if(validatorRoute){requireCompute();const[,address,action]=validatorRoute,normalized=address.toLowerCase();if(req.method==="GET"&&!action)return json(res,200,validatorRegistry.validator(normalized),true);if(req.method==="POST"&&action){const input=await body(req);if(String(input.validator||"").toLowerCase()!==normalized)throw new Error("Validator address in request does not match route");const entry=await persistCompute(()=>action==="exit"?validatorRegistry.requestExit(input):validatorRegistry.withdraw(input));return json(res,200,entry,true)}}
 if(req.method==="GET"&&url.pathname==="/api/compute/jobs"){requireCompute();return json(res,200,computeMarket.list({status:url.searchParams.get("status"),limit:url.searchParams.get("limit")||100}),true)}
 if(req.method==="POST"&&url.pathname==="/api/compute/jobs"){const input=await body(req),job=await persistCompute(()=>computeMarket.createJob(input));return json(res,201,job,true)}
 const computeRoute=url.pathname.match(/^\/api\/compute\/jobs\/([0-9a-f]{64})(?:\/(claim|result|vote|challenge|finalize|cancel|expire))?$/i);
 if(computeRoute){requireCompute();const[,jobId,action]=computeRoute;if(req.method==="GET"&&!action)return json(res,200,computeMarket.job(jobId),true);if(req.method==="POST"&&action){const input=await body(req);const result=await persistCompute(()=>action==="claim"?computeMarket.claimJob(jobId,input):action==="result"?computeMarket.submitResult(jobId,input):action==="vote"?computeMarket.vote(jobId,input):action==="challenge"?computeMarket.challengeJob(jobId,input):action==="finalize"?computeMarket.finalizeJob(jobId):action==="cancel"?computeMarket.cancelJob(jobId,input):computeMarket.expireJob(jobId));return json(res,200,result,true)}}
 if(url.pathname==="/api/jobs")return json(res,410,{error:"Prototype /api/jobs is retired; use /api/compute/jobs with KOREK_COMPUTE_MVP=1"},true);
...
apiServer.listen(NETWORK.apiPort,apiHost,()=>{console.log(`KOREK node ${NODE_VERSION} '${nodeName}'`);console.log(`Chain: ${chainName} · API/explorer: http://${apiHost}:${NETWORK.apiPort}`);console.log(`Storage: ${dataDirectory} · restored height ${chain.chain.length-1}`);console.log(`Sync: ${sync.state} · peers: ${sync.peers} (${sync.note})`);console.log(`Useful compute: ${computeEnabled?"ENABLED (single-node MVP, 10,000 KRK stake, random 3-of-5 verification, fraud-proof challenge window)":"disabled"}`);if(nodeKey)console.log(`Node identity: ${nodeKey.peerId}`);if(p2p)console.log(`P2P ${NODE_PROTOCOL} listening on http://localhost:${p2pPort}`);if(rewardsAddress)console.log(`Configured rewards: ${rewardsAddress}`)});
