...
const savedBundle=savedState?.bundleVersion===1&&savedState.networkId===NETWORK.networkId?savedState:null;
...
let preservedCompute=savedBundle?.compute||null;
const computeMarket=computeEnabled?new ComputeMarket(chain,preservedCompute):null;
const nodeSnapshot=()=>({bundleVersion:1,networkId:NETWORK.networkId,chain:chain.snapshot(),compute:computeMarket?.snapshot()||preservedCompute});
const saveNodeState=()=>stateStore.save((computeEnabled||preservedCompute)?nodeSnapshot():chain.snapshot());
...
const computeStatus=()=>computeMarket?{enabled:true,mode:"single-node-mvp",consensus:false,...computeMarket.stats()}:{enabled:false,mode:"disabled",consensus:false};
...
const requireCompute=()=>{if(!computeMarket)throw new Error("Useful-compute MVP is disabled on this node; start with KOREK_COMPUTE_MVP=1")};
const persistCompute=async action=>{requireCompute();const result=action();preservedCompute=computeMarket.snapshot();await saveNodeState();return result};
