...
export class FundedFaucet {
 ...
 claim(chain,address,now=Date.now()) {
  ...
  const input={version:3,from:this.address,to:address,amount:this.amount.toString(),timestamp:now,gasPrice:'1',gasLimit:'21000',addressScheme:'wormhole-v1',publicKey:this.wallet.publicKey};
  input.signature=cryptoProvider.sign(`${input.from}|${input.to}|${input.amount}|${now}|1|21000|wormhole-v1`,this.wallet.privateKey);
