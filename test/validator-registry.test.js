...
test("proven fraud slashes 10%, credits treasury, jails an under-minimum validator and cannot slash twice",()=>{
 ...
 const remaining=BigInt(registry.validator(target).stake),exitFields={validator:wallet.address,addressScheme:"transparent-v1",timestamp:now+102};assert.equal(registry.requestExit(signed(wallet,validatorExitMessage(exitFields),exitFields),now+102).status,"exiting");registry.finalizeJob(jobId,[],null);const due={validator:wallet.address,addressScheme:"transparent-v1",timestamp:now+102+COMPUTE_VALIDATOR_EXIT_DELAY_MS},before=BigInt(chain.balance(wallet.address));registry.withdraw(signed(wallet,validatorWithdrawMessage(due),due),due.timestamp);assert.equal(BigInt(chain.balance(wallet.address)),before+remaining);
});
