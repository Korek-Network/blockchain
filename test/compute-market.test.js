...
import {
 ComputeMarket,COMPUTE_WORKER_SHARE_BPS,computeJobMessage,computeClaimMessage,computeResultMessage,computeVoteMessage,computeCancelMessage,
...
const makeJob=(market,creator,now,payment="1000000000",extra={})=>{
 const fields={creator:creator.address,addressScheme:"transparent-v1",workloadProfile:"sha256-batch-v1",inputHash:hash("input"),payment,deadline:now+60_000,verificationThreshold:2,timestamp:now,...extra};
...
test("compute escrow settles existing KRK without minting new supply",()=>{
 ...
 chain.claimFaucet(creator.address,100n*100_000_000n,now-3_600_000);const creatorBefore=BigInt(chain.balance(creator.address)),minedBefore=chain.minedSupply,job=makeJob(market,creator,now,"1000000000");
...
test("two verifier rejections refund the customer and do not pay worker",()=>{
 ...
 chain.claimFaucet(creator.address,100n*100_000_000n,now-3_600_000);const before=chain.balance(creator.address),job=makeJob(market,creator,now,"500000000");claim(market,job,worker,now+1);result(market,job,worker,now+2);vote(market,job,v1,now+3,false,hash("different-1"));const rejected=vote(market,job,v2,now+4,false,hash("different-2"));
...
test("creator, worker and duplicate verifiers cannot self-approve settlement",()=>{
 const chain=new KorekChain(),market=new ComputeMarket(chain),creator=cryptoProvider.createWallet(),worker=cryptoProvider.createWallet(),verifier=cryptoProvider.createWallet(),now=2_400_000_000_000;
 chain.claimFaucet(creator.address,100n*100_000_000n,now-3_600_000);const job=makeJob(market,creator,now);claim(market,job,worker,now+1);result(market,job,worker,now+2);
 const creatorFields={jobId:job.id,verifier:creator.address,addressScheme:"transparent-v1",outputHash:hash("output"),approve:true,timestamp:now+3};assert.throws(()=>market.vote(job.id,signed(creator,computeVoteMessage(creatorFields),creatorFields),now+3),/cannot verify/);
 vote(market,job,verifier,now+4);const duplicate={jobId:job.id,verifier:verifier.address,addressScheme:"transparent-v1",outputHash:hash("output"),approve:true,timestamp:now+5};assert.throws(()=>market.vote(job.id,signed(verifier,computeVoteMessage(duplicate),duplicate),now+5),/already voted/);
});
