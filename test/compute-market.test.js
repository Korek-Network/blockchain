...
test("expired and cancelled jobs refund escrow and release validator assignments exactly once",()=>{
 const now=2_400_000_000_000,{chain,registry,market}=setup(now),creator=cryptoProvider.createWallet(),job=makeJob(market,chain,creator,now+100,"250000000",{deadline:now+1100}),beforeAfterEscrow=BigInt(chain.balance(creator.address));assert.throws(()=>market.expireJob(job.id,now+1000),/not reached/);const expired=market.expireJob(job.id,now+1101);assert.equal(expired.status,"expired");assert.equal(BigInt(chain.balance(creator.address)),beforeAfterEscrow+250_000_000n);job.verifiers.forEach(address=>assert.equal(registry.isAssigned(job.id,address),false));assert.throws(()=>market.expireJob(job.id,now+1102),/already final/);
 const creator2=cryptoProvider.createWallet(),job2=makeJob(market,chain,creator2,now+2000,"100000000"),cancelFields={jobId:job2.id,creator:creator2.address,addressScheme:"transparent-v1",timestamp:now+2001},cancelled=market.cancelJob(job2.id,signed(creator2,computeCancelMessage(cancelFields),cancelFields),now+2001);assert.equal(cancelled.status,"cancelled");job2.verifiers.forEach(address=>assert.equal(registry.isAssigned(job2.id,address),false));
});
